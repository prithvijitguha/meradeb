...
SCHEMA_DIR="$HOME/.local/share/gnome-shell/extensions"
...
for extension in \
  ...
  auto-move-windows@gnome-shell-extensions.gcampax.github.com; do
  schema="$SCHEMA_DIR/$extension/schemas"

  if [ -d "$schema" ]; then
    sudo cp "$schema"/*.gschema.xml \
