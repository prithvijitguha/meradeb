...
if [ ! -d "$HOME/.config/nvim" ]; then
  mkdir -p "$HOME/.config"
  cp -R "$HOME/.local/share/meradeb/configs/neovim" \
    "$HOME/.config/nvim"
  ...
  cp "$HOME/.local/share/meradeb/configs/neovim/transparency.lua" \
    "$HOME/.config/nvim/plugin/after/"
  # Default to Tokyo Night theme
  cp "$HOME/.local/share/meradeb/themes/tokyo-night/neovim.lua" \
    "$HOME/.config/nvim/lua/plugins/theme.lua"
  # Ensure editor.neo-tree is used by default
  cp "$HOME/.local/share/meradeb/configs/neovim/lazyvim.json" \
    "$HOME/.config/nvim/"
fi

# NEOVIM DESKTOP ENTRY
if [ -d "$HOME/.local/share/applications" ]; then
  if [ -f "$HOME/.local/share/meradeb/applications/Neovim.sh" ]; then
    sudo rm -f /usr/share/applications/nvim.desktop
    sudo rm -f /usr/local/share/applications/nvim.desktop
    source "$HOME/.local/share/meradeb/applications/Neovim.sh"
  fi
fi
...
if [ ! -d "$HOME/.config/.vim" ]; then
  ...
  cp "$HOME/.local/share/meradeb/configs/.vimrc" \
    "$HOME/.vimrc"
  cp -R "$HOME/.local/share/meradeb/configs/.vim/" \
    "$HOME/.config/.vim/"
fi
